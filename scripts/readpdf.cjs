...
while (pos < str.length) {
  ...
  if (after.startsWith('Tj') || after.startsWith('TJ')) {
    const raw = str.slice(start + 1, end).trim();
    if (raw.length > 2) texts.push(raw.substring(0, 300));
  }
  pos = end + 1;
}
console.log('Total text items:', texts.length);
texts.slice(0, 150).forEach((t, i) => console.log(i, JSON.stringify(t)));
