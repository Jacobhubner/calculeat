...
for (const code of codes) {
  ...
  if (!codeCell) {
    ...
    console.log(code + ': code cell not found in MCID. Raw context:');
    console.log(d.slice(Math.max(0,p-200), p+100));
    continue;
  }
  ...
  if (descCells.length === 0) {
    const nextCells = cells.filter(c => c.mcid > codeCell.mcid && c.mcid <= codeCell.mcid + 5);
    console.log(code + ': no desc cells. Code mcid=' + codeCell.mcid + ' next cells: ' + JSON.stringify(nextCells));
    continue;
  }
  ...
  console.log(code + ' [' + match + '] PDF: ' + full);
