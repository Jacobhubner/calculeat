// Stitch adjacent page streams, then do MCID extraction — handles cross-stream BDC blocks
const fs = require('fs');
...
const smIdx2 = chunk.indexOf('stream');
let ds2 = 223109 + smIdx2 + 6;
...
// Group into pages: streams within 200 bytes of each other form one page
const pages = [];
let lastEs = -1;
for (const s of allStreams) {
  if (lastEs >= 0 && s.ds - lastEs <= 300) {
    pages[pages.length-1].push(s);
  } else {
    pages.push([s]);
  }
  lastEs = s.ds + 1; // approximate
}
...
for (const code of codes) {
  // Find which page group contains this code
  const pgIdx = pages.findIndex(pg => pg.some(s => s.dec.includes('(' + code + ')')));
  if (pgIdx === -1) { console.log(code + ': not found'); continue; }

  // Stitch this page + next page (for cross-page MCID blocks)
  const stitchPages = pages.slice(pgIdx, pgIdx + 2);
  const stitched = stitchPages.flatMap(pg => pg.map(s => s.dec)).join('\n');
  const normalized = stitched.replace(/\]\s*\r?\n\s*TJ/g, '] TJ');

  // MCID extraction on stitched content
  ...
  if (!codeCell) {
    console.log(code + ': code cell not found in stitched MCID');
    continue;
  }
  ...
  if (descCells.length === 0) {
    const next = cells.filter(c => c.mcid > codeCell.mcid && c.mcid <= codeCell.mcid + 5);
    console.log(code + ': no desc cells. Next: ' + JSON.stringify(next.map(c => ({mcid:c.mcid,x:c.x,text:c.text.slice(0,30)}))));
    continue;
  }
