...
// Find the MCID block for 15135's description using MCID approach with stream stitching
const allStreams = [];
let pos = 0;
...
const codeStreamIdx = allStreams.findIndex(s => s.dec.includes('(15135)'));
const stitched = allStreams.slice(Math.max(0, codeStreamIdx-1), codeStreamIdx + 4).map(s => s.dec).join('\n');
...
const codeCell = cells.find(c => c.text === '15135');
if (!codeCell) { console.log('15135 code cell not found'); process.exit(); }
console.log('Code cell:', JSON.stringify(codeCell));
...
console.log('Desc cells:', JSON.stringify(descCells));

const full = descCells.map(c => c.text).join(' ').replace(/\s+/g, ' ').trim();
const en = JSON.parse(fs.readFileSync('public/locales/en/met.json', 'utf8')).activities['15135'];
console.log('\nPDF:', full);
console.log('EN: ', en);
console.log('PDF hex:', Buffer.from(full.slice(0, 15)).toString('hex'));
console.log('EN  hex:', Buffer.from(en.slice(0, 15)).toString('hex'));
console.log('Match:', full === en);
