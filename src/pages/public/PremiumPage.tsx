...
export default function PremiumPage() {
  ...
  const productSchema = {
    ...
    brand: { '@type': 'Organization', name: 'CalculEat', url: 'https://calculeat.com' },
    offers: [
      {
        '@type': 'Offer',
        price: '45',
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="flex-1">
        <div className="container mx-auto px-4 py-12 max-w-3xl">
          ...
          <h1 className="text-3xl md:text-4xl font-bold text-neutral-900 mb-4 flex items-center gap-3">
            <Sparkles className="h-8 w-8 text-amber-500" aria-hidden="true" />
            {t('premium.h1')}
          </h1>

          <p className="text-lg text-neutral-600 leading-relaxed mb-10">{t('premium.intro')}</p>
          ...
          <div className="grid gap-4 sm:grid-cols-2 mb-4">
            <div className="rounded-2xl border-2 border-neutral-200 p-6">
              ...
              <p className="text-sm text-neutral-500">{t('premium.plans.monthlyPeriod')}</p>
            </div>
            <div className="relative rounded-2xl border-2 border-amber-400 bg-gradient-to-br from-amber-50 to-orange-50 p-6">
              ...
              <p className="text-sm font-medium text-amber-700">{t('premium.plans.yearlyName')}</p>
              <p className="mt-2 text-4xl font-bold text-neutral-900">
                {t('premium.plans.yearlyPrice')}
              </p>
              <p className="text-sm text-neutral-500">{t('premium.plans.yearlyPeriod')}</p>
            </div>
          </div>

          <p className="flex items-center gap-2 text-sm text-neutral-600 mb-8">
          ...
          <div className="mb-12">
            ...
            <p className="mt-2 text-sm text-neutral-500">{t('premium.plans.ctaHint')}</p>
          ...
          <h2 className="text-xl font-semibold text-neutral-900 mb-4">{t('premium.tableTitle')}</h2>
          <div className="overflow-hidden rounded-xl border border-neutral-200 mb-6">
            <div className="grid grid-cols-[1.2fr_0.9fr_1.2fr] bg-neutral-50 text-sm font-semibold">
              <div className="px-4 py-3" />
              <div className="px-3 py-3 text-center text-neutral-500">
                {tPremium('upgradeModal.compare.freeHeader')}
              </div>
              <div className="flex items-center justify-center gap-1 bg-amber-100 px-3 py-3 text-amber-700">
            ...
            {COMPARE_ROWS.map(row => (
              ...
              >
                <div className="px-4 py-3 font-medium text-neutral-700">
                  {tPremium(`upgradeModal.compare.rows.${row}.label`)}
                </div>
                <div className="px-3 py-3 text-center text-neutral-500">
                  {tPremium(`upgradeModal.compare.rows.${row}.free`)}
                </div>
                <div className="bg-amber-50 px-3 py-3 text-center font-medium text-amber-800">
          ...
          <p className="text-sm text-neutral-500">{t('premium.dataPromise')}</p>
        </div>
      </main>
