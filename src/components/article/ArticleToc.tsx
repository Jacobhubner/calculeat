...
export function ArticleToc({ items, heading, variant }: ArticleTocProps) {
  ...
  if (variant === 'inline') {
    return (
      <details
        data-layout-text
        className="group mb-10 rounded-xl border border-neutral-200 bg-neutral-50 xl:hidden"
      >
        <summary className="flex items-center gap-2.5 px-4 py-3 text-sm font-medium text-neutral-800 cursor-pointer list-none [&::-webkit-details-marker]:hidden">
          ...
          <ol className="space-y-1.5 border-l border-neutral-200 pl-4">
            {items.map(item => (
              <li key={item.id}>
                <a
                  href={`#${item.id}`}
                  className="block text-sm text-neutral-600 hover:text-primary-700 transition-colors py-0.5"
                >
                  {item.label}
                </a>
              </li>
  ...
  return (
    <aside data-layout-text className="hidden xl:block">
      <nav className="sticky top-10 pl-10 pr-4 text-sm">
        <p className="text-xs font-semibold uppercase tracking-[0.08em] text-neutral-400 mb-3">
          {heading}
        </p>
