-- Byt kollation från "und-x-icu" (språkneutral) till "sv-x-icu" (svensk)
-- i search_food_items ORDER BY name, så att Å/Ä/Ö sorteras sist (svensk alfabetisk ordning)
-- i stället för efter Z (engelska ordningen som und-x-icu ger).
...
COMMENT ON FUNCTION public.search_food_items IS
  'Server-side paginering och sökning för food_items.
   ...
   Kollation: sv-x-icu (svensk — Å/Ä/Ö sorteras sist, korrekt för svenska livsmedelsnamn).
