-- Fix: recipe food_items ska alltid lagra per-100g i calories/fat_g/carb_g/protein_g
-- Tidigare skrevs perServing-värden till dessa kolumner — fel i create/update_shared_list_recipe.
-- Invariant: calories = per-100g, kcal_per_unit = per-portion.
...
BEGIN
  ...
  IF p_nutrition IS NOT NULL THEN
    ...
    ELSE
      -- portion-format: calories/fat_g/carb_g/protein_g lagras per-100g
      INSERT INTO public.food_items (
        user_id, shared_list_id, is_recipe, name,
        default_amount, default_unit,
        calories, protein_g, carb_g, fat_g,
...
-- DB-constraint: för portion-recept ska kcal_per_gram ≈ calories/100
-- Tolerans 0.01 för avrundning. Skyddar mot framtida regressioner vid INSERT/UPDATE.
ALTER TABLE public.food_items DROP CONSTRAINT IF EXISTS chk_recipe_kcal_per_gram_consistent;
ALTER TABLE public.food_items ADD CONSTRAINT chk_recipe_kcal_per_gram_consistent
  CHECK (
    NOT (
      is_recipe = true
      ...
      AND ABS(kcal_per_gram - calories / 100.0) > 0.02
    )
  );

-- DB-constraint: för portion-recept ska kcal_per_unit ≈ calories/100 * grams_per_piece
ALTER TABLE public.food_items DROP CONSTRAINT IF EXISTS chk_recipe_kcal_per_unit_consistent;
ALTER TABLE public.food_items ADD CONSTRAINT chk_recipe_kcal_per_unit_consistent
  CHECK (
    NOT (
      is_recipe = true
      AND default_unit = 'portion'
      AND kcal_per_unit IS NOT NULL
      AND calories IS NOT NULL
      AND grams_per_piece IS NOT NULL
      AND grams_per_piece > 0
      AND ABS(kcal_per_unit - (calories / 100.0 * grams_per_piece)) > 1.0
    )
  );
