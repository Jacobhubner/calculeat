-- Fix: send_share_invitation visade profile_name ("Profilkort 1") istället för username.
-- Root cause: COALESCE(profile_name, email) hoppade över username-fältet.
-- Fix: ändra till COALESCE(username, profile_name, email) — samma ordning som get_conversations().
