-- Add fiber_g to barcode_lookup_cache and update related functions

ALTER TABLE public.barcode_lookup_cache
...
BEGIN
  ...
  IF p_status = 'found' AND p_data IS NOT NULL THEN
    INSERT INTO public.barcode_lookup_cache (
      ...
      saturated_fat_g, sugars_g, salt_g, fiber_g, default_unit, food_type, status, fetched_at
    )
    VALUES (
      p_barcode,
      p_data->>'name',
  ...
  ELSIF p_status = 'not_found' THEN
    ...
    INSERT INTO public.scan_usage (user_id, scan_type, success, error_type)
    VALUES (v_user_id, 'barcode', false, COALESCE(p_error, 'off_not_found'));
  END IF;
END;
$$;
...
BEGIN
  ...
  IF v_cached.status = 'not_found' THEN
    RETURN jsonb_build_object('hit', true, 'error', 'off_not_found');
  END IF;
