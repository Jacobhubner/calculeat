-- Sök bland alla användare, med prenumerationsstatus och historik.
--
-- Ersätter behovet av att gå via en supporttråd: superadmin kan nu hitta
-- vem som helst, även den som aldrig hört av sig.
...
RETURNS TABLE (
  ...
  note               text,
  /** AKTIV Stripe-prenumeration — hanteras i betalflödet, inte här */
  is_stripe          boolean,
  /** Har någon gång betalat (även om det är uppsagt nu) */
  has_paid_before    boolean,
  /** Har någon gång haft provperiod */
  had_trial          boolean,
  /** Har någon gång fått premium tilldelad av superadmin */
  was_granted        boolean,
  /** Är admin — de är alltid founder oavsett prenumeration */
  is_admin           boolean
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    ...
    EXISTS (SELECT 1 FROM public.admins a WHERE a.user_id = up.id)
  ...
  -- Den som har något att visa först: aktiv plan, sedan senast skapad.
  ORDER BY
...
-- Historik för en enskild användare
