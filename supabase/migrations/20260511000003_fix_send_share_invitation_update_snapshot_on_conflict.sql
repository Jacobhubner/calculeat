-- Fix: send_share_invitation — uppdatera snapshot vid omsändning av pending invitation
-- Root cause: idx_share_invitations_unique_pending (partial unique index på
-- sender_id, recipient_id, item_type, item_id WHERE status='pending') fick
-- ON CONFLICT DO NOTHING att återanvända gamla snapshots vid omsändning.
-- Gamla snapshots saknade nutrients/snapshot_version och körde gammal kod.
-- Fix: DO UPDATE SET snapshot = EXCLUDED.snapshot så snapshottet alltid är färskt.
