-- Publicering till receptbanken öppnas för ALLA admins, inte bara superadmin.
--
-- Spärren låg på tre ställen som måste ändras tillsammans: de två RPC:erna
-- och triggern på recipes. Ändras bara RPC:n stoppas publiceringen ändå av
-- triggern, med ett felmeddelande som pekar åt fel håll.
--
-- Kropparna är oförändrade från 20260721050000 så när som på
-- behörighetskontrollen: `is_super_admin = true` → medlem i admins.
...
BEGIN
  IF NEW.visibility = 'official'
    ...
    IF auth.uid() IS NOT NULL AND NOT EXISTS (
      ...
      RAISE EXCEPTION 'Unauthorized: only admins can publish official recipes';
    END IF;
  END IF;
  RETURN NEW;
END;
...
REVOKE EXECUTE ON FUNCTION public.publish_recipe_to_bank FROM anon;
